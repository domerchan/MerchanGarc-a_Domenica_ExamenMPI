...
if rank == 0:
    ...
    for i in range(1, size):
        ...
        comm.send(4, dest=i, tag=2)
        comm.send(8, dest=i, tag=3)
        inicio += numFilas
    resultsPar = np.array([])    
    for i in range(1,size):
        res = np.array(comm.recv(source=i, tag=1))
        #print(res)
        resultsPar = np.concatenate((resultsPar, res), axis=None)
    finPar = time.time()
    resultsPar.sort()
    #print(resultsPar)
...
if rank > 0:
    ...
    minimum = comm.recv(source=0, tag=2)
    maximum = comm.recv(source=0, tag=3)
